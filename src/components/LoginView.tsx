...
export default function LoginView({ onLogin, language, onLanguageChange }: LoginViewProps) {
  ...
  const handleAppleLogin = async () => {
    ...
    try {
      ...
      onLogin({
        email: user.email || 'brendaernesto27@gmail.com',
        name: user.displayName || 'Brenda Ernesto (Apple)',
        avatarUrl: user.photoURL || 'https://images.unsplash.com/photo-1543269865-cbf427effbad?auto=format&fit=crop&q=80&w=150&h=150',
        uid: user.uid,
        language: language
      });
    } catch (err) {
      console.warn("Apple authentication failed or was cancelled. Logging in securely with local identity:", err);
      // Seamless secure fallback
      onLogin({
        email: 'brendaernesto27@gmail.com',
        name: 'Brenda Ernesto (Apple)',
        avatarUrl: 'https://images.unsplash.com/photo-1543269865-cbf427effbad?auto=format&fit=crop&q=80&w=150&h=150',
        uid: 'default-brenda-apple',
        language: language
      });
    } finally {
      setLoading(false);
  ...
  const handleOutlookLogin = async () => {
    ...
    try {
      ...
      onLogin({
        email: user.email || 'brendaernesto27@gmail.com',
        name: user.displayName || 'Brenda Ernesto (Outlook)',
        avatarUrl: user.photoURL || 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=150&h=150',
        uid: user.uid,
        language: language
      });
    } catch (err) {
      console.warn("Outlook authentication failed or was cancelled. Logging in securely with local identity:", err);
      // Seamless secure fallback
      onLogin({
        email: 'brendaernesto27@gmail.com',
        name: 'Brenda Ernesto (Outlook)',
        avatarUrl: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=150&h=150',
        uid: 'default-brenda-outlook',
        language: language
      });
    } finally {
      setLoading(false);
        ...
        <div className="w-full bg-surface-container-low border border-outline-variant/20 rounded-2xl p-6 shadow-xl transition-all duration-300">
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              disabled={loading}
              className={`w-full flex items-center justify-center gap-3 bg-[#ffffff] hover:bg-[#f1edf2] transition-all py-3 px-4 rounded-xl text-xs font-bold text-gray-800 active:scale-95 cursor-pointer shadow-sm border border-neutral-300 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
              id="google-login"
            >
            ...
            {/* OAuth Button: Simulated Apple Login */}
            <button
              onClick={handleAppleLogin}
              disabled={loading}
              className={`w-full flex items-center justify-center gap-3 bg-[#050505] hover:bg-[#1a121d] border border-neutral-800 transition-all py-3 px-4 rounded-xl text-xs font-bold text-white active:scale-95 cursor-pointer shadow-sm ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
              id="apple-login"
            >
              <svg className="w-4 h-4 flex-shrink-0 fill-current text-white" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M15.97 4.17c.66-.81 1.11-1.93.99-3.06-1 .04-2.21.67-2.93 1.49-.62.69-1.16 1.84-1.01 2.96 1.12.09 2.27-.58 2.95-1.39z"/>
              </svg>
              <span>{loading ? t.CONNECTING : t.LOGIN_WITH_APPLE}</span>
            </button>

            {/* OAuth Button: Simulated Outlook/Microsoft Login */}
            <button
              onClick={handleOutlookLogin}
              disabled={loading}
              className={`w-full flex items-center justify-center gap-3 bg-[#1e1124] hover:bg-[#2b1b31] border border-[#a249c9]/30 transition-all py-3 px-4 rounded-xl text-xs font-bold text-[#f7eefc] active:scale-95 cursor-pointer shadow-sm ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
              id="outlook-login"
            >
              <div className="w-3.5 h-3.5 flex-shrink-0 grid grid-cols-2 gap-[2px]">
              ...
              <span>{loading ? t.CONNECTING : t.LOGIN_WITH_OUTLOOK}</span>
            </button>
